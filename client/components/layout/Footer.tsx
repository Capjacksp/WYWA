...
const partners = [
  { name: "GRAVIKY LABS", logo: null },
  { name: "AI KIRAN", logo: null },
] as const;
      ...
      <div className="mx-auto">
        ...
        <div className="flex items-center justify-between px-[50px] py-8 max-md:flex-col max-md:gap-6 max-md:items-start border-b border-white/20">
          ...
          >
            {footerNavLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className="text-sm tracking-[0.15em] text-white/80 hover:text-white transition-colors uppercase font-body"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-8 max-md:gap-6">
            <span className="text-sm tracking-[0.15em] text-white/60 uppercase font-body">
              OUR PARTNERS
            </span>
            {partners.map((partner) => (
              ...
              >
                {/* Partner logo placeholder — replace with <img> when assets are available */}
                <span className="w-5 h-5 rounded-full bg-white/20 inline-block" />
                {partner.name}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between px-[50px] py-6 max-md:flex-col max-md:gap-4 max-md:items-start">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm tracking-[0.15em] text-white/80 uppercase font-body">
              CONNECT
            </span>
          </div>

          {/* Right — copyright */}
          <p className="text-xs tracking-[0.1em] text-white/50 uppercase font-body">
